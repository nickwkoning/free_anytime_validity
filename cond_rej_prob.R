...
cond_t <- function(n, N, X, alpha) {
  ...
  beta <- sum(X[1:n]) / sqrt(n) / sqrt(sum(X[1:n]^2))  # Current statistic
...
# for a given n, N, X, and alpha
cond_z <- function(n, N, X, alpha, sd, log_scale) {
  crit <- qnorm(alpha, log.p = log_scale, lower.tail = FALSE)
  
  if (n == N) {
    phi <- ifelse(sqrt(N) * mean(X) / sd > crit, 1, 0)
    
    if (log_scale) {
      phi <- ifelse(sqrt(N) * mean(X) / sd > crit, 0, 1e-9)
    }
  } else {
    phi <- pnorm(
      (sqrt(N / (N - n)) * crit - 1 / sqrt(N - n) * sum(X[1:n]) / sd),
      log.p = log_scale,
      lower.tail = FALSE
    )
  }
