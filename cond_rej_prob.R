# Function to compute conditional rejection probability of one-sided t-test, for a
#given n, N, X, and alpha:
cond_t <- function(n, N,X, alpha) {
  ...
  integrand <- function(w) {
    r = (N-n)/N
    x <- (sqrt(n)/sqrt(N) * beta * sqrt(w) - crit) / sqrt(r*(1-w))
...
# Function to compute conditional rejection probability of one-sided z-test, for a
#given n, N, X, and alpha:
cond_z = function(n,N,X,alpha,sd){
  ...
  if(n==N){
    ...
    phi = 1 - pnorm((sqrt(N / (N - i)) * crit - 1 / sqrt(N - i) * sum(X[1:i])/sd))
  }
  return (phi)
}
